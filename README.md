...
- **Convenience builders** — Python helpers for common workout patterns (`build_run_intervals`, `build_bike_intervals`, …)
...
| `build_bike_intervals`  | Warmup + N × (work + rest) + optional GA1 fill block + cooldown |
| `build_run_easy`        | Single time-based work step in an HR zone             |
| `build_bike_easy`       | Single time-based work step in a power range          |
| `build_swim`            | Single time-based step, no target                     |
